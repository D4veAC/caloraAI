...
export function normalizeAnalysis(result: any) {
  ...
  const items = Array.isArray(result.items) ? result.items.slice(0, 20).map((item: any) => ({
  ...
  return {
    ...
    sugar: number(result.sugar),
    sodium: number(result.sodium),
    confidence: Math.min(1, number(result.confidence)),
    summary: String(result.summary || '').slice(0, 500),
    advice: String(result.advice || '').slice(0, 500)
  };
}

export function parseModelOutput(outputContent: unknown) {
  const text = Array.isArray(outputContent)
    ? outputContent.map((part: any) => typeof part === 'string' ? part : part?.text || '').join('')
...
export class AnalyzeService {
  private async requestAnalysis(inputContent: unknown) {
  ...
  async analyzeFoodPhoto(imageBuffer: Buffer, mimeType: string, caption = '') {
    if (!/^image\/(jpeg|png|webp)$/.test(mimeType)) throw new Error('Format gambar harus JPEG, PNG, atau WebP.');
    if (imageBuffer.length > 5 * 1024 * 1024) throw new Error('Foto terlalu besar. Maksimum 5 MB.');
...
Nilai confidence harus 0..1. Hasil akan ditinjau dan dapat diedit pengguna sebelum disimpan.
Balas hanya JSON dengan bentuk: ${outputSchema}`;
    return this.requestAnalysis(prompt);
  }
}
